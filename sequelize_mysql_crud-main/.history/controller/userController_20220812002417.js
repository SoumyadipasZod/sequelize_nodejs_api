...
exports.login = async(req, res) =>{
    ...
    if(user){
        ...
        if(password_valid){
            token = jwt.sign({ "id" : user.id,"email" : user.email,"first_name":user.first_name },'soumyadip',{ expiresIn: "3m" });
...
exports.me = (req,res) => {
    async(req,res,next)=>{
        ...
        async(req,res,next)=>{
          let user = await RegModel.findOne({where:{id : req.user.id},attributes:{exclude:["password"]}});
          if(user === null){
